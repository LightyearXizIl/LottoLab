...
<style scoped>
.my-view{display:grid;gap:1.1rem}.eyebrow{margin:0;color:var(--ink-muted);font-size:.67rem;font-weight:800;letter-spacing:.1em}h1{margin:.3rem 0;font-size:1.55rem}header p:not(.eyebrow){margin:0;color:var(--ink-soft);font-size:.85rem}.tabs{display:grid;grid-template-columns:1fr 1fr;gap:.25rem;padding:.25rem;border:1px solid var(--line);border-radius:.75rem;background:#eef2f7}.tabs button{min-height:2.75rem;border:0;border-radius:.55rem;background:transparent;color:var(--ink-soft);font:inherit;font-weight:700}.tabs button.active{background:#fff;color:var(--ink);box-shadow:0 1px 5px rgb(15 23 42 / 10%)}.nested-view:deep(> header){display:none}
</style>
