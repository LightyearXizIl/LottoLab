...
const props = defineProps<{ game: GameKind, strategy: StrategyConfig, filters: FilterConfig }>()
...
<template>
  <aside class="panel">
    <div class="panel-heading"><div><p class="eyebrow">RESEARCH SETUP</p><h2>研究策略</h2></div><button class="sync" type="button" title="同步官方数据" @click="emit('sync')">↻ 同步</button></div>
    ...
    <button class="generate" type="button" :disabled="strategy.id !== 'random' && weightTotal !== 100" @click="emit('generate')">生成 5 组研究号码 <span>→</span></button>
  </aside>
</template>
...
.panel-heading { display: flex; align-items: flex-start; justify-content: space-between; } h2 { margin: .1rem 0 0; font-size: 1.05rem; }.eyebrow,.section-label { margin: 0; font-size: .67rem; font-weight: 800; letter-spacing: .1em; color: var(--ink-muted); }.sync { border: 1px solid var(--line); border-radius: .5rem; background: white; padding: .4rem .55rem; color: var(--ink-soft); cursor: pointer; font: inherit; font-size: .78rem; }.field { display: grid; gap: .35rem; color: var(--ink-soft); font-size: .78rem; font-weight: 650; }.field input,.field select { width: 100%; box-sizing: border-box; border: 1px solid var(--line); border-radius: .5rem; padding: .55rem .6rem; color: var(--ink); background: #fff; font: inherit; }.weights,.section { display: grid; gap: .7rem; padding-top: .95rem; border-top: 1px solid var(--line); }.weight-heading { display: flex; justify-content: space-between; font-size: .78rem; color: var(--ink-soft); }.weight-heading b { color: #18885f; }.weight-heading b.warn { color: #bf5b11; }.range { display: grid; grid-template-columns: 5.5rem 1fr 2.3rem; align-items: center; gap: .5rem; font-size: .72rem; color: var(--ink-soft); }.range input { accent-color: #d82f45; }.range output { text-align: right; font-variant-numeric: tabular-nums; }.split { display: grid; grid-template-columns: 1fr 1fr; gap: .55rem; }.generate { display: flex; justify-content: space-between; align-items: center; border: 0; border-radius: .65rem; padding: .78rem .85rem; background: linear-gradient(135deg, #cc2842, #a71632); color: #fff; cursor: pointer; font: inherit; font-weight: 750; box-shadow: 0 7px 16px rgb(190 24 60 / 22%); }.generate:disabled { cursor: not-allowed; opacity: .55; }
</style>
